...
reliable protocol, since unacknowledged packets are sent again. Many application layer protocols
use TCP for transport. For example, SSH, HTTPS, FTP and other all use TCP as the base, since these
protocols do not tolerate the packet losses. 
...
Most important UDP protocols and ports is shown in Figure~\ref{fig:udp}.
...
Internet Control Message Protocol is tipycally used to detect liveness of the hosts in the Internet,
...
(DNS) into IPv4 and IPv6 addresses. It is also used to perform reverse transformation from
...
the name in the query, and DNS server sends next query to TLD; Finally, the DNS server sends
