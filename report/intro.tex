...
need to troubleshoot the network quite often. For that matter, in the paragraphs that follow we discuss basic network
...
the query to the root; root domain server responds with the Top Level Domain (TLD) server IP responsible for
the name in the query, and DNS server sends next query to TLD DNS server; Finally, the DNS server sends
