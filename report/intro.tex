...
All these metrics can be used while measuring the performance of the networked systems. In the paragraphs that follow
...
need to be synchronized. Technically, of course we can also use sophisticated algorithms and packet trains to measure one way 
...
throughput can service larger number of clients in the network. {\it Goodput} is the same as throughput, but excludes 
...
use notion of reliability (that is of the data is corrupted it is requested again) and, hence, high error rate can 
...
congestion. But we are not going to cover those in this report.

\section{Topologies}

There are several key topologies that are used in enterprises: mesh, star and hybrid. {\it Mesh} 
topology is when every network element is connected to every other network element. The links can be 
...
will remain and packets will be delivered, not to all but some devices. 
