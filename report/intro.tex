...
Most important UDP protocols and ports is shown in Table~\ref{tab:udp}.
...
Keeping time synchnorized in the network is important. Many applications rely on accurate timing.
For example, application loggging system puts the timestamps in the files, precise activation of scheduled jobs relies on 
...
NTP operates on UDP port 123, so it is imporatant that firewall will allow this port to be open.
...
In addition we have monitored the  long-living VPN connections. Upon doing so, we have noticed that 
service providers do not play nicely with such connections.
For exampl, we have observed that TCP connections can enter so called 
